...
static uint8_t server_num_clients = 0;

static PacketInfo packet_info[PACKET_INFO_MAX_LEN] = {0};
static uint32_t packet_info_index = 0;
...
static uint32_t get_ack_bit_field(NodeInfo* node_info)
{
    ...
    for(;;)
    {
        ...
        if(server_num_clients > 0)
        {
            ...
            for(int i = 0; i < server_num_clients; ++i)
            {
                ...
                memcpy(send_packet.data,(uint8_t*)&world_state,sizeof(world_state));
